...
export default function App() {
  ...
  const loadNewCustomer = async (
    ...
    const targetIdMap: Record<number, string[]> = {
      0: ["🍢 关东煮", "🥤 冰镇可乐"],
    ...
    const targetsForGuest = targetIdMap[nextGuestId] || [];
    ...
    if (rotatedItemIds.length === 0 || forceResetItems || !isLangChange) {
      ...
      const targeted = allIds.filter(id => targetsForGuest.includes(id));
      const others = allIds.filter(id => !targetsForGuest.includes(id));
      
      let finalRotatedIds: string[] = [];
      let attempts = 0;
      
      while (attempts < 20) {
        // We MUST include all requested elements under targetsForGuest
  ...
  return (
    <div className="flex flex-col bg-[#FFEECC] min-h-screen text-[#442211] font-sans p-2 sm:p-4 select-none">
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div 
            ...
            className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-full font-black shadow-[4px_4px_0px_0px_rgba(102,68,34,1)] border-4 border-[#664422] flex items-center gap-2 text-sm sm:text-base animate-pulse ${
              toastType === 'success' 
          ...
          >
            {toastType === 'success' ? (
              ...
              <Info className="w-5 h-5 text-[#7A5C00]" />
            )}
      ...
      {/* Top Header Panel */}
      <header className="max-w-5xl w-full mx-auto flex flex-col md:flex-row justify-between items-center bg-[#FFD54F] border-8 border-[#664422] rounded-[32px] p-5 mb-5 gap-4 shadow-[8px_8px_0px_0px_rgba(102,68,34,1)] text-[#442211]" id="game-header">
        
        {/* Market Title */}
        <div className="flex items-center gap-3">
          <div className="bg-[#FF8A65] p-2.5 rounded-2xl border-4 border-[#664422] shadow-[2px_2px_0px_0px_rgba(102,68,34,1)]">
            <Store className="w-6 h-6 text-white" />
          </div>
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-[#442211] tracking-tight flex items-center gap-2 drop-shadow-sm">
              {t.gameName}
              <span className="text-xs font-mono font-black bg-[#664422] text-white border-2 border-[#664422] rounded-lg px-2 py-0.5">Vibrant Palette v1.0</span>
            </h1>
            <p className="text-xs text-[#664422]/90 font-bold">{t.gameSubtitle}</p>
          </div>
        </div>

        {/* Global Progress Heart Bar */}
        <div className="flex flex-col items-center flex-grow max-w-sm w-full bg-white px-4 py-2.5 rounded-[24px] border-4 border-[#664422] shadow-[4px_4px_0px_rgba(102,68,34,0.1)]">
          ...
          <div className="w-full bg-[#664422] h-6 rounded-full overflow-hidden border-4 border-[#664422] relative p-0.5">
            <motion.div 
          ...
          <div className="flex justify-between w-full text-[10px] text-[#664422] mt-1 font-bold font-mono">
      ...
      <main className="w-full max-w-5xl mx-auto flex flex-col items-center">
        ...
        {aiOfflineMode && (
          <div className="w-full mb-4 bg-white border-8 border-[#664422] text-[#664422] rounded-[24px] p-4 text-xs font-bold flex items-center gap-2 shadow-[6px_6px_0px_rgba(102,68,34,0.1)]">
            <Info className="w-4 h-4 text-[#FF8A65] shrink-0" />
            <span>
              {t.offlineStatusBanner}
            </span>
          </div>
        )}
        ...
        <section 
          className="w-full aspect-[16/9] rounded-[32px] overflow-hidden border-[12px] border-[#664422] shadow-[12px_12px_0px_0px_rgba(102,68,34,1)] bg-[#A1D6E2] relative flex flex-col justify-between"
        ...
        >
          ...
          {satisfaction <= 30 && (
            <div className="absolute top-2 right-4 bg-red-600/90 text-[10px] font-mono border-2 border-white text-white font-extrabold py-1 px-3.5 rounded-full z-10 animate-pulse flex items-center gap-1 shadow-lg">
          ...
          {satisfaction >= 90 && satisfaction < 100 && (
            <>
              ...
              <div className="absolute top-2 right-4 bg-[#FFD54F] text-[10px] font-mono border-2 border-[#664422] text-slate-950 font-extrabold py-1 px-3.5 rounded-full z-30 animate-bounce flex items-center gap-1 shadow-lg">
          ...
          {/* Scene Top Banner */}
          <p className="z-10 text-[11px] font-extrabold bg-[#FFD54F] text-[#442211] border-b-4 border-[#664422] px-4 py-1.5 text-center select-none shadow-[0_2px_4px_rgba(0,0,0,0.1)]">
              ...
              <AnimatePresence mode="wait">
                {!loadingGuest && activeGuest ? (
                  <motion.div 
                    ...
                    exit={{ x: -80, opacity: 0 }}
                    className="h-full max-h-full aspect-square flex items-end justify-center relative group -translate-x-28 translate-y-[45px]"
                  >
            ...
            <div className="w-[45%] h-full flex flex-col justify-end items-center relative">
              <AnimatePresence mode="wait">
                  <motion.div 
                    ...
                    exit={{ opacity: 0 }}
                    className="h-full max-h-full aspect-square flex items-end justify-center relative translate-x-28 translate-y-[45px]"
                  >
          ...
          <div className="absolute bottom-0 inset-x-0 h-[38%] bg-gradient-to-b from-[#8E7066] to-[#6A4D43] border-t-8 border-[#563A31] pt-2 pb-1.5 px-3 sm:px-4 flex justify-between gap-3 sm:gap-4 z-20 shadow-[inset_0_4px_8px_rgba(0,0,0,0.3)] select-none">
            ...
            <div className="w-[49%] h-[92%] flex flex-col select-text">
              <motion.div 
                ...
                key={customerMessage}
                className="w-full h-full bg-white text-[#442211] rounded-2xl p-2 sm:p-2.5 border-4 border-[#664422] flex flex-col shadow-[4px_4px_0_rgba(102,68,34,0.15)] overflow-hidden"
              >
                {/* Speaker info bar */}
                <div className="flex items-start justify-between gap-2 mb-1 border-b-2 border-[#664422]/20 pb-0.5 font-black shrink-0">
                  <span className="text-[#664422] text-[9px] sm:text-[11px] leading-tight flex items-start gap-1 min-w-0">
                  ...
                  <span className="bg-[#FF8A65] border-2 border-[#664422] text-white px-1.5 py-0.5 rounded-lg text-[8px] sm:text-[9.5px] font-black uppercase shrink max-w-[50%] text-center break-words whitespace-normal leading-normal font-sans">
                    {customerFeeling}
                  </span>
                </div>

                {/* Speech content */}
                <p className="text-[#442211] break-all leading-snug sm:leading-relaxed font-black text-[9.5px] sm:text-[11px] md:text-xs flex-grow overflow-y-auto pr-1">
                  {transitioningNext ? (
                    <span className="text-[#2E7D32] animate-pulse flex flex-col justify-center items-start gap-1 py-1">
            ...
            <div className="w-[49%] h-[92%] flex flex-col select-text">
              <motion.div 
                ...
                className="w-full h-full bg-[#E0F2F1] text-[#442211] rounded-2xl p-2 sm:p-2.5 border-4 border-[#664422] flex flex-col shadow-[4px_4px_0_rgba(102,68,34,0.15)] overflow-hidden"
              >
                {/* Speaker info bar */}
                <div className="flex items-start justify-between gap-2 mb-1 border-b-2 border-[#664422]/20 pb-0.5 font-black shrink-0">
                  <span className="text-[#664422] text-[9px] sm:text-[11px] leading-tight flex items-start gap-1 min-w-0">
                  ...
                  <span className="bg-[#4DB6AC] border-2 border-[#664422] text-white px-1.5 py-0.5 rounded-lg text-[8px] sm:text-[9.5px] font-black uppercase shrink max-w-[50%] text-center break-words whitespace-normal leading-normal font-sans">
                    {t.statusOnline}
                  </span>
                </div>

                {/* Speech content */}
                <p className="text-[#1A3038] font-black break-all leading-snug sm:leading-relaxed text-[9.5px] sm:text-[11px] md:text-xs flex-grow overflow-y-auto pr-1">
                  {shopkeeperMessage}
                </p>
              </motion.div>
            </div>
          ...
          {satisfaction === 100 && (
            <div className="absolute inset-0 bg-[#664422]/85 backdrop-blur-md z-40 flex flex-col items-center justify-center p-4">
              <motion.div 
                initial={{ scale: 0.85, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="max-w-md w-full bg-white rounded-[32px] border-8 border-[#4CAF50] p-6 text-center shadow-2xl relative text-[#442211]"
              >
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-[#FFD54F] text-[#442211] font-black px-6 py-2.5 rounded-full border-8 border-[#664422] shadow-[6px_6px_0px_0px_rgba(102,68,34,1)] text-sm sm:text-base md:text-lg animate-bounce whitespace-nowrap">
                  {t.successTitle}
                </div>
                ...
                <div className="h-44 flex items-center justify-center my-4">
                  {getShopkeeperImageSrc() ? (
                    <img 
                      src={getShopkeeperImageSrc()} 
                      alt="Game Success" 
                      className="h-full w-auto object-contain filter drop-shadow-[0_8px_16px_rgba(102,68,34,0.3)] animate-pulse"
                    />
                  ) : (
                ...
                <h3 className="text-lg sm:text-xl font-black text-[#442211]">{t.successSubtitle}</h3>
                <p className="text-xs text-[#664422] mt-2 mb-4 font-bold leading-relaxed">
                  {t.successDesc}
                </p>
                ...
                <div className="flex gap-4">
                  <button 
                    ...
                    className="flex-grow bg-[#4CAF50] hover:bg-[#388E3C] text-white font-black text-xs sm:text-sm py-4 px-6 rounded-2xl transition duration-150 border-4 border-[#664422] shadow-[4px_4px_0px_0px_rgba(102,68,34,1)] cursor-pointer active:translate-y-0.5"
                  >
                    {t.successNextBtn}
                  </button>
                </div>
          ...
          {satisfaction === 0 && (
            <div className="absolute inset-0 bg-[#FF5D5D]/90 backdrop-blur-md z-40 flex flex-col items-center justify-center p-4">
              <motion.div 
                initial={{ scale: 0.85, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="max-w-md w-full bg-white rounded-[32px] border-8 border-[#FF5D5D] p-6 text-center shadow-2xl relative text-[#442211]"
              >
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-[#FF5D5D] text-white font-black px-6 py-2.5 rounded-full border-8 border-[#664422] shadow-[6px_6px_0px_0px_rgba(102,68,34,1)] text-sm sm:text-base md:text-lg flex items-center gap-1.5 animate-pulse whitespace-nowrap">
                ...
                <h3 className="text-lg sm:text-xl font-black text-red-500">{t.failedSubtitle}</h3>
                <p className="text-xs text-[#664422] mt-2 mb-4 font-bold leading-relaxed">
                  {t.failedDesc}
                </p>
                ...
                <div className="flex gap-4">
                  <button 
                    ...
                    className="flex-grow bg-[#FF7043] hover:bg-[#D84315] text-white font-black text-xs sm:text-sm py-4 px-6 rounded-2xl transition duration-150 border-4 border-[#664422] shadow-[4px_4px_0px_0px_rgba(102,68,34,1)] cursor-pointer active:translate-y-0.5"
                  >
                    {t.failedRetryBtn}
                  </button>
                </div>
        ...
        <section className="w-full max-w-5xl mt-4 flex flex-col sm:flex-row gap-3 justify-between items-center bg-[#FFF8E1] border-4 border-[#664422] rounded-2xl px-5 py-3 text-xs text-[#442211] shadow-[4px_4px_0px_0px_rgba(102,68,34,1)] animate-fade-in">
          <div className="flex items-center gap-1.5 font-black text-slate-800">
            <span className="text-[#664422]">{t.currentGuestLabel}</span> 
            <strong className="text-rose-600 font-black bg-white px-2.5 py-1 rounded-lg border-2 border-[#664422]/20">
              {activeGuest?.guestName || (lang === "en" ? "Loading..." : lang === "ko" ? "로딩 중..." : "加载中")}
            </strong>
            <span className="text-[#664422]/70 font-semibold">
              {lang === "en" ? `(${dialogRoundCount} interactive rounds)` : lang === "ko" ? `(${dialogRoundCount}회 대화 진행됨)` : `(${dialogRoundCount} 个交互回合)`}
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <button 
              ...
              className="bg-white hover:bg-[#FFE0B2] font-black border-4 border-[#664422] px-3 py-1.5 rounded-xl text-[#664422] shadow-[2px_2px_0px_rgba(102,68,34,1)] transition cursor-pointer active:translate-y-0.5 disabled:opacity-50"
            >
              {t.switchLin}
            </button>
            <button 
            ...
            <button 
              ...
              className="bg-[#4DB6AC] hover:bg-[#26A69A] text-white border-4 border-[#664422] font-black text-[11px] px-3.5 py-1.5 rounded-xl transition duration-150 shadow-[2px_2px_0px_rgba(102,68,34,1)] cursor-pointer active:translate-y-0.5 disabled:opacity-50 font-sans"
            >
              {t.randomGuest}
            </button>
          </div>
        ...
        <section 
          className="w-full max-w-5xl mt-5 bg-[#FFF8E1] border-8 border-[#664422] rounded-[32px] p-5 sm:p-6 shadow-[12px_12px_0px_0px_rgba(102,68,34,1)] text-[#442211]"
        ...
        >
          ...
          <div className="mb-4">
            <div className="flex justify-between items-center mb-3">
              <h2 className="text-xs sm:text-sm font-black text-[#664422] flex items-center gap-1.5 uppercase tracking-wide">
                <span>{t.itemShelfHeader}</span>
              </h2>
              {selectedItems.length > 0 && (
                <button 
                  onClick={clearSelection}
                  className="text-[10px] sm:text-xs font-black text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 border-2 border-red-500 rounded-xl px-2.5 py-1 transition flex items-center gap-1 cursor-pointer active:scale-95 font-sans"
                >
              ...
              {(() => {
                ...
                return (
                  <button
                    ...
                    className={`text-left rounded-2xl p-2.5 border-4 transition duration-150 cursor-pointer relative flex flex-col justify-between h-[105px] ${
                      isSelected 
                        ? "bg-[#FFD54F] border-[#D84315] text-[#442211] shadow-[3px_3px_0px_rgba(216,67,21,1)] scale-[1.02] -translate-y-0.5 font-bold" 
                        : "bg-white hover:bg-[#FFE0B2] border-[#664422] text-[#442211] shadow-[3px_3px_0px_rgba(102,68,34,1)] active:translate-y-0.5"
                    }`}
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xl sm:text-2xl drop-shadow-sm shrink-0">{item.emoji}</span>
                      <div className="min-w-0">
                        <span className="text-xs font-black block leading-tight truncate text-[#442211]">{item.name}</span>
                        <span className="text-[10px] font-mono font-black bg-[#FFEECC] border-2 border-[#664422] px-1 rounded-md text-[#442211] mt-1 inline-block">{item.cost}</span>
                      </div>
                    </div>
                    <p className="text-[10px] text-[#664422]/80 leading-normal block truncate select-none" title={item.desc}>
                      {item.desc}
                    </p>
                    {isSelected && (
                      <div className="absolute top-1.5 right-1.5 w-3 h-3 rounded-full bg-[#D84315] border-2 border-white animate-pulse" />
                    )}
                  </button>
                );
              })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-5">
            ...
            <div className="md:col-span-1 bg-white p-3.5 rounded-2xl border-4 border-[#664422] shadow-[3px_3px_0px_rgba(102,68,34,0.1)]">
              <h3 className="text-xs font-black text-[#664422] mb-2 uppercase tracking-wide">{t.quickRepliesHeader}</h3>
              <div className="flex flex-col gap-1.5">
                {quickReplies.map((reply, idx) => (
                  <button
                    key={idx}
                    onClick={() => insertTemplate(reply)}
                    className="w-full text-left font-black text-[11px] bg-[#FFF8E1] hover:bg-[#FFE0B2] border-2 border-[#664422] p-2.5 rounded-xl text-[#664422] transition duration-100 truncate active:scale-[0.99] cursor-pointer"
                    title={reply}
                  >
                    {reply}
                  </button>
            ...
            <div className="md:col-span-2 flex flex-col justify-between bg-white p-4 rounded-2xl border-4 border-[#664422] shadow-[3px_3px_0px_rgba(102,68,34,0.1)]">
              
              {/* Counter status area */}
              <div className="flex justify-between items-center mb-2">
                <h4 className="text-xs font-black text-[#664422] tracking-wide">{t.customRepliesHeader}</h4>
                <div className="flex flex-wrap items-center gap-1">
                  {selectedItems.length === 0 ? (
                    <span className="text-[10px] text-[#664422]/60 font-black italic">{t.noItemsPacked}</span>
                  ) : (
                    <div className="flex items-center gap-1 flex-wrap">
                      <span className="text-[10px] text-rose-600 font-extrabold shrink-0 mr-1">{t.packedLabel}</span>
                      {selectedItems.map((item, idx) => {
                        const activeItems = LOCALIZED_ITEMS[lang];
                        const found = activeItems.find(x => x.id === item);
                        const labelText = found ? `${found.emoji} ${found.name}` : item;
                        return (
                          <span key={idx} className="bg-[#FF8A65] border border-[#664422] text-white font-black text-[10px] px-2 py-0.5 rounded-lg flex items-center shrink-0">
                            {labelText}
                          </span>
                        );
                      })}
              ...
              <div className="flex-grow flex gap-3">
                <textarea
                  ...
                  className="flex-grow bg-[#FFF8E1] border-4 border-[#664422] rounded-xl p-3 text-xs md:text-sm text-[#442211] placeholder-[#664422]/50 focus:outline-none focus:bg-[#FFF] focus:ring-2 focus:ring-[#FFD54F] resize-none h-24 font-bold transition disabled:opacity-65"
                  onKeyDown={(e) => {
                ...
                <button
                  disabled={loadingGuest || evaluateLoading || transitioningNext}
                  onClick={handleServe}
                  className="w-24 bg-[#FF7043] hover:bg-[#FF8A65] hover:scale-102 active:scale-95 text-white font-black rounded-2xl border-4 border-[#664422] flex flex-col items-center justify-center gap-2 shadow-[4px_4px_0px_rgba(102,68,34,1)] hover:shadow-[2px_2px_0px_rgba(102,68,34,1)] hover:translate-x-0.5 hover:translate-y-0.5 transition duration-150 cursor-pointer disabled:opacity-50 shrink-0 uppercase"
                >
                  {evaluateLoading ? (
                    ...
                    <>
                      ...
                      <span className="text-[11px] font-black uppercase shrink-0">{t.serveBtn}</span>
                    </>
                  )}
                </button>
              </div>
              ...
              <div className="flex justify-between items-center mt-2 text-[10px] text-[#664422]/70 font-bold">
      ...
      {/* Decorative Footer */}
      <footer className="mt-8 text-center text-[#664422] text-xs py-4 select-none font-bold leading-relaxed">
        <p>{t.footerLine1}</p>
        <p className="mt-1 font-mono text-[10px] text-[#664422]/70">{t.footerLine2}</p>
      </footer>
    </div>
  );
}
