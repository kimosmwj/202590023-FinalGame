...
export const LOCAL_ITEM_NAMES: Record<string, Record<string, string>> = {
...
export function getOfflineDialogueAndTargets(
  guestId: number, 
  ...
  // We randomly increase required items by 1 to 2 over several rounds to make difficulty higher
  const augmentedTargets = [...baseTargets];
  let finalSpeech = customerSpeechBase;

  // 50% chance to request extra items to raise difficulty in offfline mode
  if (Math.random() < 0.5) {
    const rawPool = [
...
export function getLocalizedOfflineQuickReplies(lang: string, guestId: number, targets?: string[]): string[] {
  const currentLang = (lang === "en" || lang === "ko") ? lang : "zh";
  ...
  const lastItem = getItemDisplayName(finalTargets[finalTargets.length - 1] || "🥤 冰镇可乐", currentLang);
  ...
  if (currentLang === "en") {
    ...
    if (guestId === 0) {
      return [
        ...
        `따끈따끈 오동통한 ${joinedItems}을(를) 정성껏 준비했습니다! 최고의 명탐정님, 오늘도 화이팅! 🍢`,
